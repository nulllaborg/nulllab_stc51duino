...
#define InitialUARTx(x) \
  ...
  REN = 1;      \
  SM2 = 0;\
  attachInterrupt(4,UART0_IRQHandler,0); \
  ES = 1;\
  EA = 1

#define RXREG SBUF
void UART0_IRQHandler(void) // void Serial_ISR(void) __interrupt 4
{
  uint8_t c;
  if (RI)
  { /* if reception occur */
...
#define InitialUARTx(x)  \
  ...
  S2CON |= ( S2REN);\
  S2CON &= (~S2SM2);\
  attachInterrupt(UART2_INTNO,UART2_IRQHandler,0); \
  IE2 |= ( ES2);\
  EA   = 1

#define RXREG S2BUF
void UART2_IRQHandler(void))  // void Serial1_ISR(void) __interrupt UART2_INTNO
{
  uint8_t c;
  if ( (S2CON & S2RI) == S2RI )
  { /* if reception occur */
